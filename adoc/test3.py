...
Square = namedtuple("Square", "xmin xmax ymin ymax")
...
def put_path(path, field, order, dx=0, dy=0):
    for pp in path:
        #print(pp)
        #print_field(field)
...
def create_field(path1, path2):
    ...
    yy = [pp.y for pp in path1]
    minx, maxx = min(xx), max(xx)
    miny, maxy = min(yy), max(yy)

    xx = [pp.x for pp in path2]
    yy = [pp.y for pp in path2]
    minx, maxx = min(min(xx), minx), max(max(xx), maxx)
    miny, maxy = min(min(yy), miny), max(max(yy), maxy)

    print("init field, size = {} x {}".format(maxy - miny + 1, maxx - minx + 1))
    field = []
...
def get_min_dist(field, center):
    min_dist = -1
    for y in range(len(field)):
        for x in range(len(field[0])):
            if field[y][x][0] == 1 and field[y][x][1] == 1:
...
def get_result(wire1, wire2):
    print("analyze path1")
    path1 = get_path(wire1)
    print("analyze path2")
    path2 = get_path(wire2)
    print("done")
    field, center = create_field(path1, path2)
    dist = get_min_dist(field, center)
    return dist
...
if __name__ == "__main__":
    ...
    for wire1, wire2, res in tests:
        dist = get_result(wire1.split(","), wire2.split(","))
        print(dist, res)
