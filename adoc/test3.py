...
def get_path(wire):
    ...
    for move in wire:
        ...
        for _ in range(steps):
            ...
            y += dy
            cnt += 1
            pos.append(Point(x=x, y=y, steps=cnt))
    return pos


def put_path(path, field, order):
    if order not in (0, 1):
        raise ValueError
    for pp in path:
        if (pp.y, pp.x) not in field:
            field[(pp.y, pp.x)] = {}
        if order not in field[(pp.y, pp.x)]:
            field[(pp.y, pp.x)][order] = pp.steps
        else:
            field[(pp.y, pp.x)][order] = min(field[(pp.y, pp.x)][order], pp.steps)
...
def prepare_field(wire1, wire2):
    path1 = get_path(wire1)
    path2 = get_path(wire2)
    field = {}
    put_path(path1, field, 0)
    put_path(path2, field, 1)
    return field


def get_min_dist(wire1, wire2):
    field = prepare_field(wire1, wire2)
    min_dist = -1
    for pp, val in field.items():
...
def get_min_path(wire1, wire2):
    field = prepare_field(wire1, wire2)

    min_path = -1
    for pp, val in field.items():
        if 0 in val and 1 in val:
            path_len = val[0] + val[1]
            if path_len > 0 and (min_path < 0 or path_len < min_path):
                min_path = path_len
    return min_path
...
if __name__ == "__main__":
    ...
    tests = [("R8,U5,L5,D3", "U7,R6,D4,L4", 6, 30),
    ...
    for wire1, wire2, res, res2 in tests:
        dist = get_min_dist(wire1.split(","), wire2.split(","))
        print(dist, res)

        path_len = get_min_path(wire1.split(","), wire2.split(","))
        print(path_len, res2)
